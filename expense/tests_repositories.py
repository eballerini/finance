...
from .factories import AccountFactory, CreditCardFactory, UserFactory, TransactionFactory, TransactionImportFactory
...
class TransactionRepositoryTests(TestCase):
    ...
    def setUp(self):
        ...
        self.transaction1 = TransactionFactory(date_added=date(2019, 12, 12), account_id=self.account.id, transaction_import_id=self.import1.id)
        self.transaction2 = TransactionFactory(date_added=date(2018, 11, 12), account_id=self.account.id, transaction_import_id=self.import1.id)
        self.transaction3 = TransactionFactory(date_added=date(2020, 1, 2), account_id=self.account.id)
        self.transaction4 = TransactionFactory(date_added=date(2020, 1, 2), account_id=self.another_account.id)
